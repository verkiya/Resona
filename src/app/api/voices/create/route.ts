// REST endpoint for custom voice creation — validates subscription, audio metadata, uploads to S3, then persists objectKey on the Voice row.
...
export async function POST(request: Request) {
  ...
  // voice creation is gated behind an active org subscription because this is a paid feature.
  try {
  ...
  // the upload is rejected early unless we can prove the file is a real audio file with enough duration to be useful.
  let duration: number;
  try {
  ...
  try {
    ...
    if (createdVoiceId) {
      // if upload or metadata persistence fails, we delete the partially created voice so the database does not keep a broken record.
      await prisma.voice
        .delete({
          where: {
            id: createdVoiceId,
  ...
  // metering is fire-and-forget so billing telemetry cannot slow down or fail the voice creation response.
  polar.events
    .ingest({
      events: [
        {
